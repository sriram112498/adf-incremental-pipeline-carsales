...
df_silver = spark.sql("SELECT * FROM PARQUET.`abfss://silver@cardobbydatalake.dfs.core.windows.net/carsales`")
...
if spark.catalog.tableExists('factsales'):
    ...
    deltatb.alias('trg').merge(df_fact.alias('src'), 'trg.dim_branch_key = src.dim_branch_key and trg.dim_dealer_key = src.dim_dealer_key and trg.dim_model_key = src.dim_model_key and trg.dim_date_key = src.dim_date_key')\
    ...
    df_fact.write.format('delta')\
            .mode('overwrite')\
            .option("path","abfss://gold@cardobbydatalake.dfs.core.windows.net/factsales")\
